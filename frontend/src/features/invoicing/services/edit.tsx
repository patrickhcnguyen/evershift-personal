    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const deleteStaffPromises = deletedStaffUuids.map(async (uuid) => {
                ...
                const response = await fetch(`${process.env.VITE_SERVER_URL}/api/staff-requirements/${uuid}`, {
                    method: 'DELETE',
                });
            ...
            const staffPromises = editedStaff.map((staff, index) => {
                const baseUrl = `${process.env.VITE_SERVER_URL}/api/staff-requirements`;
            ...
            const requestResponse = await fetch(`${process.env.VITE_SERVER_URL}/api/requests/${editedInvoice.request_id}`, {
                method: 'PUT',
            ...
            const invoiceResponse = await fetch(`${process.env.VITE_SERVER_URL}/api/invoices/${editedInvoice.uuid}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            try {
                const ratesResponse = await fetch(`${process.env.VITE_SERVER_URL}/api/rates/${editedInvoice.request_id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                ...
                const finalInvoiceResponse = await fetch(`${process.env.VITE_SERVER_URL}/api/invoices/${editedInvoice.uuid}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
